...
std::string Response::Head_Response()
{
	try
	{
		...
		this->path += url;
		auto len = get_file_length();
		auto type = get_file_type();
		return ok_headers(OK, len, type);
	}
	catch (std::exception& e)
	{
		std::cerr << e.what() << std::endl;
...
std::string Response::current_time()//->boost::posix_time::ptime
{
	boost::posix_time::ptime timeLocal = boost::posix_time::second_clock::local_time();
	return to_simple_string(timeLocal);
}
...
{
	char* data = (char*)malloc(sizeof(char)* get_file_length());
	//auto path = root + url;
